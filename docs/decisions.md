...
Decision: OutboxEvent row in same @Transactional as task + history + audit, poller SELECT WHERE published=false LIMIT 100 every 5s, builds DomainEvent record and sync kafka send with header eventId + get 3s, markPublished.
